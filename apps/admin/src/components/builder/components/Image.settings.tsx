...
function ImageContentSettings({
  ...
  props: ImageProps & { dynamicBindings?: DynamicBindings };
  setProp: (cb: (props: ImageProps & { dynamicBindings?: DynamicBindings }) => void) => void;
}) {
  ...
  const srcBinding = props.dynamicBindings?.src;
  const hasDynamicSrc = !!srcBinding;

  const handleSrcBindingChange = (binding: DynamicBinding | undefined) => {
    setProp((p) => {
      if (!p.dynamicBindings) p.dynamicBindings = {};
      if (binding) {
        p.dynamicBindings.src = binding;
      } else {
        delete p.dynamicBindings.src;
      }
    });
  };
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <DynamicTagButton
              binding={srcBinding}
              onBindingChange={handleSrcBindingChange}
              valueTypeFilter={['image', 'url']}
            />
          </div>
          {hasDynamicSrc ? (
            <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3">
              <p className="text-xs text-amber-600 dark:text-amber-400">
                Bound to <span className="font-mono font-medium">{srcBinding.field}</span>
              </p>
              {srcBinding.fallback && (
                <p className="text-[10px] text-muted-foreground mt-1">
                  Fallback: {srcBinding.fallback}
                </p>
              )}
            </div>
          ) : (
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => setProp((p) => (p.objectFit = value as ImageProps['objectFit']))}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
